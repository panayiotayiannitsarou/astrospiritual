...
def generate_section5_aspects_with_openai(payload: dict) -> str:
    ...
    system_prompt = """Είσαι έμπειρη αστρολόγος.
Θέλω να γράψεις ΜΟΝΟ την ΕΝΟΤΗΤΑ 5 – Όψεις, χωρισμένη σε τρεις υποενότητες:

5. ΕΝΟΤΗΤΑ 5 – Όψεις

5Α. Βασικές ψυχολογικές όψεις
Όψεις που περιλαμβάνουν Ήλιο, Σελήνη, Ωροσκόπο (AC), κυβερνήτη Ωροσκόπου.
Για κάθε μία όψη: 3–4 προτάσεις, συγκεκριμένες.

5Β. Θεραπευτικές / καρμικές όψεις
Όψεις Χείρωνα, Βόρειου Δεσμού, Κρόνου, Πλούτωνα.
Για κάθε όψη: 3–5 προτάσεις, με θεραπευτικό τόνο.

5Γ. Λοιπές όψεις
Όψεις μεταξύ των υπόλοιπων πλανητών.
Περιγραφές 2–4 προτάσεων.

ΣΗΜΑΝΤΙΚΟ:
- Γράψε ξεχωριστή παράγραφο για ΚΑΘΕ όψη που υπάρχει στο JSON.
- Μην τις συγχωνεύεις.
- Όψεις που ΔΕΝ υπάρχουν, αγνόησέ τες."""

    user_prompt = f"""Γράψε ΜΟΝΟ την Ενότητα 5 – Όψεις με τις υποενότητες 5Α, 5Β, 5Γ.
...
def main():
    ...
    st.markdown("Για κάθε ζευγάρι πλανητών, αν υπάρχει σημαντική όψη, διάλεξε τη μορφή της.")
    ...
    for i, (gr1, en1) in enumerate(PLANETS):
        ...
        for j in range(i + 1, len(PLANETS)):
            ...
            choice = st.selectbox(f"{gr1} – {gr2}", aspect_labels, key=key)
