...
def generate_section4_report_with_openai(payload: dict) -> str:
    ...
    system_prompt = (
        ...
        "Διαίρεσε την ενότητα στις παρακάτω υποενότητες, με ακριβώς αυτούς τους τίτλους και αρίθμηση:\n\n"
        ...
        "  * τι τον βοηθά να συνδεθεί με τον αληθινό του εαυτό,\n"
...
def generate_section5_aspects_with_openai(payload: dict) -> str:
    """
    Μόνο Ενότητα 5 – Όψεις (με υποενότητες 5Α, 5Β, 5Γ).
    Αναλυτική περιγραφή κάθε όψης ξεχωριστά.
    """
    client = get_openai_client()
    if client is None:
        return (
    ...
    system_prompt = (
        "Είσαι έμπειρη αστρολόγος.\n"
        "Λαμβάνεις ως είσοδο ένα JSON με δομή γενέθλιου χάρτη που περιλαμβάνει aspects (όψεις).\n"
        ...
        "— Για κάθε μία όψη: γράψε 3–4 προτάσεις, συγκεκριμένες.\n"
        "— Εξήγησε πώς επηρεάζει τον χαρακτήρα, τη βασική ψυχολογία του ατόμου.\n"
        "— Κάθε όψη σε ξεχωριστή, μικρή παράγραφο.\n\n"
        "5Β. Θεραπευτικές / καρμικές όψεις\n"
        "— Όψεις Χείρωνα (συμπεριλαμβανομένων Χείρωνας–AC, Χείρωνας–MC), Βόρειου Δεσμού, Κρόνου, Πλούτωνα.\n"
        "— Εδώ μπορεί να «στρώσει» πιο βαθιά ανάλυση για πληγές, μάθημα, εξέλιξη.\n"
        "— Για κάθε όψη: 3–5 προτάσεις, με θεραπευτικό και ενδυναμωτικό τόνο.\n"
        "— Κάθε όψη σε ξεχωριστή παράγραφο.\n\n"
        "5Γ. Λοιπές όψεις\n"
        "— Όψεις μεταξύ των υπόλοιπων πλανητών (Ερμής, Αφροδίτη, Άρης, Δίας, Ουρανός, Ποσειδώνας κ.λπ.).\n"
        "— Περιγραφές 2–4 προτάσεων, καθαρές & συγκεκριμένες.\n"
        "— Κάθε όψη σε ξεχωριστή, μικρή παράγραφο.\n\n"
        "ΣΗΜΑΝΤΙΚΟ:\n"
        "- Γράψε ξεχωριστή, μικρή παράγραφο για ΚΑΘΕ όψη που υπάρχει στο JSON.\n"
        "- Μην τις συγχωνεύεις όλες σε μία γενική παράγραφο.\n"
        "- Όψεις που ΔΕΝ υπάρχουν στο JSON, αγνόησέ τες.\n"
        "- Αν υπάρχουν πολλές όψεις (>15), δώσε λίγο μεγαλύτερη ανάλυση στις πιο σημαντικές (5Α και 5Β) "
        "και για τις λοιπές (5Γ) μπορείς να είσαι πιο σύντομος.\n\n"
        "ΓΕΝΙΚΕΣ ΟΔΗΓΙΕΣ ΥΦΟΥΣ:\n"
        "- Γράψε σε απλή, καθαρή, σύγχρονη ελληνική γλώσσα.\n"
        "- Να είναι ζεστό, ενδυναμωτικό, με σεβασμό. Όχι μοιρολατρικό.\n"
        "- Μη χρησιμοποιείς τεχνική ορολογία χωρίς εξήγηση.\n"
        "- Μην αναφέρεις μοίρες αν δεν είναι απαραίτητο.\n"
        "- Μη μιλάς για "καλό/κακό χάρτη". Μίλα για δυνατότητες, προκλήσεις και εξέλιξη.\n\n"
        "Η γλώσσα να είναι ζεστή, σαφής, παιδαγωγική και ενδυναμωτική."
    )

    user_prompt = (
        "Παρακάτω είναι τα δεδομένα του χάρτη σε JSON.\n"
        "Χρησιμοποίησέ τα για να γράψεις ΜΟΝΟ την Ενότητα 5 – Όψεις "
        "με τις υποενότητες 5Α, 5Β, 5Γ όπως περιγράφονται στο system prompt.\n\n"
        f"{json.dumps(payload, ensure_ascii=False, indent=2)}"
    )
...
def main():
    ...
    if basic_button or talents_button or aspects_button:
        ...
        if aspects_button:
            ...
            with st.spinner("Καλώ το μοντέλο μόνο για την Ενότητα 5 (Όψεις)..."):
                try:
                    report_text = generate_section5_aspects_with_openai(payload)
                except Exception as e:
                    report_text = f"Παρουσιάστηκε σφάλμα κατά την κλήση του OpenAI API:\n{e}"

            st.markdown("### 📜 Ενότητα 5 – Όψεις (5Α, 5Β, 5Γ)")
            st.write(report_text)

            st.markdown("---")
            pdf_buffer = create_pdf(payload, report_text)
            st.download_button(
                label="📄 Λήψη Αναφοράς Ενότητας 5 σε PDF",
                data=pdf_buffer,
                file_name=f"section5_aspects_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf",
                mime="application/pdf"
            )
